...
var _hLine,_vLine,_hLineLatlngs,_vLineLatlngs;
...
function caluHLineLatLng(b) {
	_hLineLatlngs = [new L.LatLng((b._northEast.lat+b._southWest.lat)/2, initBound._northEast.lng),new L.LatLng((b._northEast.lat+b._southWest.lat)/2,initBound._southWest.lng)];
}

function caluVLineLatLng(b) {
	_vLineLatlngs = [new L.LatLng(initBound._northEast.lat,(b._northEast.lng+b._southWest.lng)/2),new L.LatLng(initBound._southWest.lat,(b._northEast.lng+b._southWest.lng)/2)];
}
...
$(document).ready(function($) {
	...
	recordBarHide = 1;
	showList = 0;
	cropWidth = $("#cropper").width();
	cropHeight = $("#cropper").height()
	...
	caluHLineLatLng(initBound);
	_hLine = L.polyline(_hLineLatlngs, {color: '#ff0000',weight: 1}).addTo(miniMap);
	caluVLineLatLng(initBound);
	_vLine = L.polyline(_vLineLatlngs, {color: '#ff0000',weight: 1}).addTo(miniMap);
	...
	miniMap.on('click', function(e) {
    	...
    	caluHLineLatLng(bound);
		_hLine.setLatLngs(_hLineLatlngs);
		caluVLineLatLng(bound);
		_vLine.setLatLngs(_vLineLatlngs);
	         ...
	         success: function(data) {
	            if (data.code == 0) {
	               ...
	               for (i = 0; i < item.length; i++) {
	               	...
	               	imgSrc +=  "<a href='slice.html?display=111111111&barcode=" + item[i].barcode + "&isinternational=" + data.data.isinternational+"&imgId=" + item[i].id + "&width=" + item[i].width +"&height="+ item[i].height+ "&maxzoom="+ item[i].maxzoom+ "'><img src='" + item[i].picurl.replace("preview.jpg","macro.jpeg") + "'></a>";
	               };
	               imgSrc += "</div>";
	               $(".tools").append(imgSrc);
